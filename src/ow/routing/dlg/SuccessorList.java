 ...
 * */
	...
	*
	* */
	public IDAddressPair closestTo(String suffix) {

		synchronized (this.list) {
			for(IDAddressPair node : this.list){
				String nodeStr = utils.IdDeleteZero(node.getID());
				if (nodeStr.substring(nodeStr.length() -1).equals(suffix)){
					return node;
				}
			}
		}
